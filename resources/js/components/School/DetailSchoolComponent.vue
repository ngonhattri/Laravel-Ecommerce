...
<script>
import {SCHOOLS_DETAIL, SCHOOLS_EDIT} from '../../store/actions.type.js'
export default {
    data() {
        return {
            form:{}
    ...
    mounted(){
        this.$store.dispatch(SCHOOLS_DETAIL,this.$route.params.id);
    },
    methods:{
        openFileManager2 () {
        ...
        edit(){
            this.$store.dispatch(SCHOOLS_EDIT,{id:this.$route.params.id,data:this.$data.form})
            .then(response=>{
    ...
    computed:{
        page(){
            this.$data.form = this.$store.getters.school;
