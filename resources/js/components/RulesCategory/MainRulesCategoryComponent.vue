                            ...
                            <div class="dd dd-draghandle">
                                <ol class="dd-list" v-for="item,key in pages">
                                    <li class="dd-item dd2-item">
                                        ...
                                        <div class="dd2-content">{{item.title}}</div>
                                    </li>
                                </ol>
                            </div>
...
<script>
import { FETCH_RULESCATEGORIES,RULESCATEGORIES_CREATE,RULESCATEGORIES_DELETE } from '../../store/actions.type.js'
export default {
    data(){
        return{
            form:{
    ...
    methods:{
        ...
        deletePage(id){
            if(confirm("Bạn có muốn xóa ?")){
                this.$store.dispatch(RULESCATEGORIES_DELETE,id);
                this.fetchPageList(); 
            }
        },
        add(){
            this.$store.dispatch(RULESCATEGORIES_CREATE,this.$data.form)
            .then((response)=>{
    ...
    computed:{
        ...
        isLoading(){
            return this.$store.getters.isRulesCategoryLoading;
        }
    }
}
</script>
