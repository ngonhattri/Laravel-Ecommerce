    ...
    <div class="main-content-inner">
        <div class="breadcrumbs ace-save-state" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li class="active">Danh mục Điều lệ</li>
            </ul>
        </div>
        <div class="page-content">
            <div class="widget-box">
                <div class="widget-header">
                    <h4 class="widget-title">Quản lý điều lệ</h4>
                    <div class="widget-toolbar no-border">
                        <router-link :to="{ name: 'rules-add'}">
                        <a class="btn btn-sm btn-info">
                            <i class="ace-icon fa fa-plus"></i> Thêm mới điều lệ
                        </a>
                        </router-link>
                    </div>
                </div>
                        ...
                        <table id="simple-table" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <tr v-for="page in pages.data">
                                    ...
                                    <td>{{page.title}}</td>
                                    <td>{{page.rules_categories.title}}</td>
                                    <td>
                                        <div class="hidden-sm hidden-xs btn-group">
                                            <router-link tag="button" class="btn btn-xs btn-info" :to="`/qanivia/rule/${page.id}`">
...
<script>
import { FETCH_RULES,RULES_DELETE,RULES_MULTI } from '../../store/actions.type.js'
export default {
    data(){
        return{
            totalPages : 0,
    ...
    methods:{
        fetchPageList(pageNumber){
            ...
            this.$store.dispatch(FETCH_RULES,{pageNumber,keyword:this.$data.keyword});
        },
        deletePage(id){
            if(confirm("Bạn có muốn xóa ?")){
                this.$store.dispatch(RULES_DELETE,id);
                this.fetchPageList(); 
            }
        },
        deleteMultiple(){
            if(confirm("Bạn có muốn xóa toàn bộ ?")){
                this.$store.dispatch(RULES_MULTI,this.$data.selection);
                this.$data.selection = [];
    ...
    computed:{
        pages(){
            var value = this.$store.getters.rules;
        ...
        isLoading(){
            return this.$store.getters.isRulesLoading;
        }
    }
}
</script>
