...
import {RULES_DETAIL, RULES_EDIT,FETCH_RULESCATEGORIES} from '../../store/actions.type.js'
...
export default {
    ...
    mounted(){
        this.$store.dispatch(RULES_DETAIL,this.$route.params.id);
    },
    methods:{
        openFileManager2 () {
        ...
        edit(){
            this.$store.dispatch(RULES_EDIT,{id:this.$route.params.id,data:this.$data.form})
            .then(response=>{
    ...
    computed:{
        page(){
            this.$data.form = this.$store.getters.rule;
            return this.$store.getters.rule;
        },
        rules_category(){
