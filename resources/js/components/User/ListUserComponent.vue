    ...
    <div class="main-content-inner">
        <div class="breadcrumbs ace-save-state" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li class="active">Danh mục admin</li>
            </ul>
        </div>
        <div class="page-content">
            <div class="widget-box">
                <div class="widget-header">
                    <h4 class="widget-title">Quản lý tài khoản</h4>
                    <div class="widget-toolbar no-border">
                        <router-link :to="{ name: 'user-add'}">
                        <a class="btn btn-sm btn-info">
                            <i class="ace-icon fa fa-plus"></i> Thêm mới tài khoản
                        </a>
                        </router-link>
                    </div>
                </div>
...
import { FETCH_USERS,USERS_DELETE, USERS_MULTI } from '../../store/actions.type.js'
export default {
    data(){
        return{
            totalPages : 0,
    ...
    methods:{
        fetchPageList(pageNumber){
            ...
            this.$store.dispatch(FETCH_USERS,{pageNumber,keyword:this.$data.keyword});
        },
        deletePage(id){
            if(confirm("Bạn có muốn xóa ?")){
                this.$store.dispatch(USERS_DELETE,id);
                this.fetchPageList(); 
            }
        },
        checked(item){
        ...
        deleteMultiple(){
            if(confirm("Bạn có muốn xóa toàn bộ ?")){
                this.$store.dispatch(USERS_MULTI,this.$data.selection);
                this.$data.selection = [];
                $('#btnDelete').attr('disabled','disabled');  
                this.$data.all = false;     
                this.fetchPageList();
    ...
    computed:{
        pages(){
            var value = this.$store.getters.users;
        ...
        isLoading(){
            return this.$store.getters.isUserLoading;
        }
    }
}
</script>
