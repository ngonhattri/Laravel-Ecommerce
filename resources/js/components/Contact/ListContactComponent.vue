    ...
    <div class="main-content-inner">
        <div class="breadcrumbs ace-save-state" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li class="active">Danh mục thông tin công ty</li>
            </ul>
        </div>
        <div class="page-content">
            <div class="widget-box">
                <div class="widget-header">
                    <h4 class="widget-title">Quản lý thông tin</h4>
                    <div class="widget-toolbar no-border">
                        <router-link :to="{ name: 'contact-add'}">
                        <a class="btn btn-sm btn-info">
                            <i class="ace-icon fa fa-plus"></i> Thêm mới thông tin
                        </a>
                        </router-link>
                    </div>
                </div>
                                ...
                                <tr v-for="page in pages.data">
                                    ...
                                    <td>
                                        <div class="hidden-sm hidden-xs btn-group">
                                            <router-link tag="button" class="btn btn-xs btn-info" :to="`/qanivia/contact/${page.id}`">
...
<script>
import { FETCH_CONTACTS,CONTACTS_DELETE,CONTACTS_MULTI } from '../../store/actions.type.js'
export default {
    data(){
        return{
            totalPages : 0,
    ...
    methods:{
        fetchPageList(pageNumber){
            ...
            this.$store.dispatch(FETCH_CONTACTS,{pageNumber,keyword:this.$data.keyword});
        },
        deletePage(id){
            if(confirm("Bạn có muốn xóa ?")){
                this.$store.dispatch(CONTACTS_DELETE,id);
                this.fetchPageList(); 
            }
        },
        deleteMultiple(){
            if(confirm("Bạn có muốn xóa toàn bộ ?")){
                this.$store.dispatch(CONTACTS_MULTI,this.$data.selection);
                this.$data.selection = [];
    ...
    computed:{
        pages(){
            var value = this.$store.getters.contacts;
        ...
        isLoading(){
            return this.$store.getters.isContactLoading;
        }
    }
}
</script>
